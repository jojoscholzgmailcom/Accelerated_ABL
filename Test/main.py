...
if __name__ == "__main__":
    ...
    all_TPsOld = np.zeros((number_of_iterations,number_of_attempts))
    ...
    for itr in range(number_of_iterations):
        ...
        baf = BAF2.BAF2([4,3])
        bafOld = BAF2OLD.BAF2OLD(gen.scenario, gen)
        ...
        for attempt in range(number_of_attempts):
            ...
            guess = baf.generate_second_guess(gen.scenario_to_numerical(), saved_scenarios_in_memory_for_other_approaches, saved_best_recovery_behaviors_in_memory_for_other_approaches, show_rule=True)
            baf.update_baf(gen.scenario_to_numerical(), gen.recovery_behavior_to_numerical(), saved_scenarios_in_memory_for_other_approaches, saved_best_recovery_behaviors_in_memory_for_other_approaches)
            if gen.recovery_behavior_to_numerical() == guess:
                TP += 1
            all_TPs[itr, attempt] = TP
            guess = bafOld.generate_second_guess(gen.scenario, saved_scenarios_in_memory_for_other_approaches, saved_best_recovery_behaviors_in_memory_for_other_approaches, show_rule=True)
            bafOld.update_baf(gen.scenario, gen.best_recovery_behavior, saved_scenarios_in_memory_for_other_approaches, saved_best_recovery_behaviors_in_memory_for_other_approaches)
            if gen.best_recovery_behavior == guess:
                TP_OLD += 1
            all_TPsOld[itr, attempt] = TP_OLD
            ...
            print(f"{attempt}:Our: {TP}, Old: {TP_OLD}, Decision tree: {DT_TP}")
        ...
        ax.plot(range(number_of_attempts), np.mean(all_TPsOld[:itr+1], axis=0)/(np.array(range(number_of_attempts)) + 1), 'b-', label="ABL (old)")
        ax.plot(range(number_of_attempts), np.mean(all_TPs[:itr+1], axis=0)/(np.array(range(number_of_attempts)) + 1), 'g-', label="ABL (modified)")
