...
class ScenarioGenerator:
    ...
    def scenario_to_2D_numerical(self):
        numerical = []
        for sen in self.scenario:
            if sen[0] != "Noc":
                numerical.append([list(self.colors.keys())[list(self.colors.values()).index(sen[0])], list(self.concepts.keys())[list(self.concepts.values()).index(sen[1])] + 4])
            else:
                numerical.append([-1, -1])
        return numerical
    ...
    def scenario_to_numerical(self):
        ...
        for sen in self.scenario:
            if sen[0] != "Noc":
                ...
                numerical.append(list(self.concepts.keys())[list(self.concepts.values()).index(sen[1])] + 4)
            else:
                numerical.append(-1)
                numerical.append(-1)
        return numerical
